...
1. Determine if this company matches the user's requirements (qualified must be true or false).
   ...
   - STRICT EMPLOYER EXCLUSION: If '{current_employer}' is provided and is not null/empty, you MUST absolutely DISQUALIFY the company if they ARE the Current Employer themselves, or if the website explicitly indicates they are a subsidiary, parent company, or part of the same corporate group as the Current Employer. We only want to target competitors, never the candidate's existing employer.
